...
JUNK_Q_TO_BE_REPLACED = np.array( [0,0,0,1])
JUNK_P_TO_BE_REPLACED = np.array( [0,0,0])
JUNK_R_TO_BE_REPLACED = np.eye(3)
...
class HuskyMapper:
    ...
    def odometry_callback(self, msg):
        ...
        #
        self.q_map_baselink = np.array([self.odometry.pose.pose.orientation.x, self.odometry.pose.pose.orientation.y, self.odometry.pose.pose.orientation.z, self.odometry.pose.pose.orientation.w])
        ...
        #       multiplication, which is not usually what you want
        #
        self.p_map_baselaser = np.dot(self.R_baselink_baselaser, self.p_baselink_baselaser) + self.p_map_baselink
        self.mutex.release()
    ...
    def is_in_field_of_view(self, robot_row, robot_col, robot_theta, row, col):
        ...
        #
        x = self.ogm.meters_per_cell * (col - robot_col)
        y = self.ogm.meters_per_cell * (row - robot_row)
        d = np.sqrt(x*x + y*y)
        if d > self.max_laser_range or d < self.min_laser_range:
            return False

        a = atan2(y, x)
        angle_diff = atan2(sin(robot_theta - a), cos(robot_theta - a))
        if angle_diff > self.max_laser_angle or angle_diff < self.min_laser_angle:
            return False
        return True
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        r = np.sqrt(x*x + y*y)
        a = atan2(y, x)
        diff_angle = a - robot_theta_in_map
        diff_angle = atan2(sin(diff_angle), cos(diff_angle))
