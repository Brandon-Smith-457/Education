...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        def exp_normalize(x):
            ...
            y = np.exp(x - b)
            return y / np.sum(y)
        N_eff = exp_normalize(errors)
        print "effective sample size", N_eff

        self.weights = []
        i = 0
        for particle in self.particles:
            r, a = self.subsample_laser_scan(laser_scan)
            d = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, a, self.laser_min_range, self.laser_max_range)
            diff = []
            for i in range(len(r)):
                diff.append(r[i] - d[i])
            sigma2 = self.beam_range_measurement_noise_std_dev**2
            diffdivsigma2 = [x / sigma2 for x in diff]
            negdiffdivsigma2 = [-1 * x for x in diffdivsigma2]
            expnegdiffdivsigma2 = [np.exp(x) for x in negdiffdivsigma2]
            normexpnegdiffdivsigma2 = [N_eff * x for x in expnegdiffdivsigma2]
            self.weights.append(np.prod(normexpnegdiffdivsigma2))
            i += 1
    ...
    def resample(self):
        ...
        for particle in particles:
            ...
            p = Particle(particles[index].id, particles[index].x, particles[index].y, particles[index].theta)
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for angle in angles:
            ...
            i = 0
            while True:
                h = min_range + i*range_step
                x = h * cos(angle)
                y = h * sin(angle)
                if x <= self.xmax and x >= self.xmin and y <= self.ymax and y >= self.ymin:
                    xg, yg = self.metric_to_grid_coords(x, y)
                    if not self.grid_bin[xg][yg] or h >= max_range:
                        ranges.append(h)
                        break
                else:
                    if h >= max_range:
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        sim_ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, angles_in_upwards_baselaser_frame, self.laser_min_range, self.laser_max_range)
        ...
        for i in range(len(actual_ranges)):
            diff.append(sim_ranges[i] - actual_ranges[i])
        norm_error = np.sqrt(np.sum(np.square(diff)))
        return norm_error**2
