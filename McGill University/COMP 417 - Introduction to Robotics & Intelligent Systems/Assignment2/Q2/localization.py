...
def dynamics_model(state, control, dt):
    # TODO: fill this
    expected_state = State(state.x + control.vx * dt, state.y + control.vy * dt)     # MUST CHANGE THESE VALUES
    return expected_state

def measurement_model(state, landmark):
    # TODO: fill this
    expected_observation = Observation(landmark.id, np.sqrt(np.square(state.x - landmark.x) + np.square(state.y - landmark.y)))                # MUST CHANGE THESE VALUES
...
class LocalizationWithRangeMeasurements(object):
    ...
    def cost_function(self, x):
        ...
        F = np.array([])                                                      # MUST CHANGE THIS VALUE!
        ...
        deviation_from_init_state = self.init_state.diff(State(x[0], x[1]))                               # MUST CHANGE THIS VALUE!
        ...
        for t in xrange(1, T):
            ...
            xt = State(x[State.dim*t], x[State.dim*t + 1])
            xtminus1 = State(x[State.dim*(t-1)], x[State.dim*(t-1) + 1])
            utminus1 = self.controls_across_time[t-1]
            dc = np.array(self.dynamics_cost(xt, xtminus1, utminus1))
        ...
        for t in xrange(self.num_timesteps):
            ...
            xt = State(x[State.dim*t], x[State.dim*t + 1])
            zt = self.observations_across_time[t]
            lt = None
            for i in range(len(zt)):
                for landmark in self.landmarks:
                    if zt[i].landmark_id == landmark.id:
                        lt = landmark
                mc = np.array(self.measurement_cost(xt, lt, zt[i]))
...
if __name__ == "__main__":
    ...
    for t in xrange(num_timesteps):
        for obs in observations_across_time[t]:
            l = landmarks[int(obs.landmark_id)]
