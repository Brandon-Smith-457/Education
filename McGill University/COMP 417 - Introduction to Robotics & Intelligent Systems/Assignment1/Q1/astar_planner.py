...
class AStarPlanner(object):
    ...
    def _follow_parent_pointers(self, parents, state):
        ...
        # UNCOMMENT THE 5 LINES TO OUTPUT PATH LENGTH TO TERMINAL
        #dist = 0
        #prev_ptr = state

        while curr_ptr is not None:
            shortest_path.append(curr_ptr)
            #dist = dist + sqrt((curr_ptr.x - prev_ptr.x)**2 + (curr_ptr.y - prev_ptr.y)**2)
            #prev_ptr = curr_ptr
    ...
    # The heuristic function for astar_planner implemented as the linear distance rounded down to nearest integer
    def heuristic(self, curr_state, dest_state):
        return int(sqrt((dest_state.x - curr_state.x)**2 + (dest_state.y - curr_state.y)**2))
    ...
    def plan(self, start_state, dest_state):
        ...
        # Array that contains the priority for the queue
        f = float("inf") * np.ones((world.shape[0], world.shape[1]))

        best_dist_found[start_state.x, start_state.y] = 0
        f[start_state.x, start_state.y] = self.heuristic(start_state, dest_state)

        # Q is a mutable priority queue implemented as a dictionary
        Q = priority_dict()
        Q[start_state] = f[start_state.x, start_state.y]
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                # to visit it then update its priority in the queue usisng f, and also its
                # distance to come and its parent
                if (ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    f[ns.x, ns.y] = alternative_best_dist_ns + self.heuristic(ns, dest_state)
                    Q[ns] = f[ns.x, ns.y]
...
if __name__ == "__main__":
    ...
    dest_state = State(354, 591)

    # UNCOMMENT THE TWO PRINT STATEMENTS TO OUTPUT THE TIME STAMPS
    #print(time.clock())
