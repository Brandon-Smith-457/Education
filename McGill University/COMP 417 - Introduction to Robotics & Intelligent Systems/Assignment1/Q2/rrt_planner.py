...
from math import cos, acos, sin, pi, sqrt
...
class RRTPlanner(object):
    ...
    def sample_state(self):
        ...
        """
        cols = len(self.world)
        rows = len(self.world[0])
        x = random.randint(0, cols-1)
        y = random.randint(0, rows-1)
        x = int(round(x))
        y = int(round(y))
        assert(x >= 0 and x < cols)
    ...
    #
    # IF I HAVE TIME REMEMBER TO CHANGE THIS TO A KD TREE PARTITIONING
    #
    ...
    def find_angle(self, s_nearest, s_rand, dist):
        angle = acos((s_rand.x - s_nearest.x)/dist)
        if (s_nearest.y > s_rand.y):
            angle = 2 * pi - angle
        return angle
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        dist = s_rand.euclidean_distance(s_nearest)

        if (dist <= max_radius):
            x = s_rand.x
            y = s_rand.y
        else:
            angle = self.find_angle(s_nearest, s_rand, dist)
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        dist = s_from.euclidean_distance(s_to)
        angle = self.find_angle(s_from, s_to, dist)
        max_checks = 10
        for i in xrange(max_checks):
            dist_temp = float(i)/max_checks * dist
            x = s_from.x + cos(angle) * dist_temp
            y = s_from.y + sin(angle) * dist_temp
            x = int(round(x))
            y = int(round(y))
            s_temp = State(x, y, s_from)
            if (not self.state_is_free(s_temp)):
                return False
